...
class ANDS_RIFCS {
  ...
  */
   ...
   */  
	...
	*/
	protected 	function create_regObject($group, $key, $originatingSource,$keyurl) {
		$regObj_node = $this->addChild($this->working_node,'registryObject');
		$regObj_node->setAttribute('group',$group);
		$this->addChild($regObj_node,'key',$keyurl);
  ...
  /** Create RelatedInfo
   *  Created by Irwan Krisna Of ANU (2012)
   *
   *
  */           
          protected 	function create_relatedInfo($key2,$key,$relation_type = 'publication',$identifier_type_format = 'issn') {
		$c = $this->addChild($this->working_node, 'relatedInfo');
		$a = $this->addChild($c,'title',$key);
		//$a->setAttribute('type',$relation_type_format);
		$b = $this->addChild($c,'identifier',$key2);
		$b->setAttribute('type',$identifier_type_format);
		//$b = $this->addChild($c,'title',
		//$c = $this->addChild($c, 'notes');
	...
	protected     function create_relatedInfo3($key2,$key,$relation_type = 'publication',$identifier_type_format = 'isbn') {
		$c = $this->addChild($this->working_node, 'relatedInfo');
		$a = $this->addChild($c,'title',$key);
		$b = $this->addChild($c,'identifier',$key2);
		$b->setAttribute('type',$identifier_type_format);
	...
	/** Added by Irwan Krisna 2012-12-30 to add the website of each researcher */
	protected 	function create_relatedInfo2($key,$relation_type = 'website',$relation_type_format = 'url') {
		$c = $this->addChild($this->working_node, 'relatedInfo');
		$a = $this->addChild($c,'identifier',$key);
		//$b = $this->addChild($c,'identifier',$key2);
		$b->setAttribute('type',$relation_type_format);
		$c->setAttribute('type', $relation_type);
	}
	...
	*/
	protected 	function create_identifier_node($key, $i_type='uri') {
