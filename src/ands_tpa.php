	...
	function create_obj_node($set_name, $key) {
		...
		if (in_array($set_name,prepare_set_names())) {
			try {
				...
				$query = "select ori_table_name, ori_id,url,oai_identifier_url from oai_records where oai_identifier = '".$key."'";
			...
			$this->create_regObject(REG_OBJ_GROUP, $key, MY_URI,$record['url']);
			$this->$processor($record['ori_table_name'],$record['ori_id']);	
			$this->create_identifier_node($record['oai_identifier_url']);
...
#		$this->create_description_node(sprintf("Line one:%s,\nLine two:%s.\nThird",'a','b'));
	...
	private function create_party($table_name, $id_party) {
		$db = $this->db;
		$query = sprintf("SELECT set_type FROM oai_records WHERE ori_table_name = '%s' AND ori_id = '%s' ",$table_name,$id_party);
	...
	private 	function create_person($table_name, $id_party) {
		$db = $this->db;
		$query = sprintf("SELECT id_org, title, first_name, family_name, tel, fax, email, www, address, post_code, city,state,country,duty,description,subject,relatedInfo,idpub,for1,for1_pct,for2,for2_pct,for3,for3_pct FROM %s WHERE id_rep = '%s'",$table_name, $id_party);
		//$query2 = sprintf("SELECT distinct  publication.pubid,publication.title  FROM %s  inner join useraccount on useraccount.id_rep = publication.ori_id and useraccount.id_rep = '%s'",$table_name2, $id_party);
		$query2 = sprintf("select distinct  publication.ands_pub_id AS pubid,publication.title AS pubtitle from publication inner join %s on useraccount.id_rep = publication.ori_id and useraccount.id_rep = '%s' ",$table_name,$id_party);
		$res = exec_pdo_query($db, $query);	
		$res2 = exec_pdo_query($db, $query2);	
		$info = $res->fetch(PDO::FETCH_ASSOC);
		//$info2 = $res2->fetch(PDO::FETCH_ASSOC);
		$c = $this->create_name_node();
    ...
    // Description (additional by Irwan Krisna) 
    $d_node = $this->create_description_node($info['description']);
    
    //  Subject (additional by Irwan Krisna)
		if(!empty($info['for1'])) $s_node = $this->create_subject_node($info['for1']);
		//if(!empty($info['for1_pct'])) $s_node = $this->create_subject_node($info['for1_pct']);
		if(!empty($info['for2'])) $s_node = $this->create_subject_node($info['for2']);
		//if(!empty($info['for2_pct'])) $s_node = $this->create_subject_node($info['for2_pct']);
		if(!empty($info['for3'])) $s_node = $this->create_subject_node($info['for3']);
		//if(!empty($info['for3_pct'])) $s_node = $this->create_subject_node($info['for3_pct']);
		//$s_node = $this->create_subject_node($info['for1_pct']);
		//$s_node = $this->create_subject_node($info['for2']);
		//$s_node = $this->create_subject_node($info['for2_pct']);
		//$s_node = $this->create_subject_node($info['for3']);
		//$s_node = $this->create_subject_node($info['for3_pct']);
    ...
    // related Info (additional by Irwan Krisna)
    while ($info2 = $res2->fetch(PDO::FETCH_ASSOC)){
    	if((substr($info2['pubid'],-5,1)=="-") and !empty($info2['pubid']))
	$r_obj1 = $this ->create_relatedInfo($info2['pubid'],$info2['pubtitle']);
	if((substr($info2['pubid'],-5,1)!="-") and !empty($info2['pubid']))
	$r_obj2 = $this ->create_relatedInfo3($info2['pubid'],$info2['pubtitle']);
    	
    }	
     	//
 		//$r_obj2 = $this->create_relatedInfo2($info['www']);    
		$l_node = $this->create_location_node();
		$a_node = $this->create_address_node($l_node);
